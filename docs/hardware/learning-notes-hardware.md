...
The LED heartbeat is not only decoration. It creates emotional presence and shows Huggy's current state.
...
The KY-006 passive buzzer can create tones. It is not a speaker and cannot produce normal speech.
...
The control signal comes from the microcontroller. The servo power should come from a suitable separate supply. The grounds must share a common reference.
...
**Important:** Never assume that every servo can safely move from 0° to 180° inside a mechanism. Mechanical limits must be tested carefully.
